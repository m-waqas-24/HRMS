        ...
        <div class="page-header d-xl-flex d-block">
            <div class="page-leftheader">
                ...
                <ol class="breadcrumb1">
                    <li class="breadcrumb-item1"><a  href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item1 active">Edit Role</li>
                </ol>
            </div>
        ...
        <div class="row">
            <div class="col-xl-12 col-md-12 col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                       <form action="{{ route('admin.update.role', $role->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <label class="form-label mb-0 mt-2">Roles</label>
                                    <input type="text" name="name" class="form-control" placeholder="Enter Name" value="{{ $role->name }}" id="">
                                </div>
												...
												<div class="tab-content">
                                                    @if(getCompanyPlan()->hrm)
                                                        <div class="tab-pane active" id="tab11">
                                                                    @php
                                                                        $modules = ['employee ', 'salary', 'allowance ', 'commission', 'loan ', 'deduction ', 'otherpayments', 'leaves', 'trainer', 'traininglist', 'training-type', 'hradminsetup', 
                                                                        'transfer', 'promotions', 'awards', 'resignation', 'termination', 'complaints', 'holidays', 'recruitments', 'jobs', 'candidates', 'interviewschedule', 'employee-assets', 
                                                                        'notice-board', 'events', 'departments', 'designations', 'leave-types', 'assets ', 'contract-types', 'gifts', 'award-type', 'payslip-options', 'allowance-options', 'loan-options', 
                                                                        'deduction-options', 'attendance-overview', 'attendance-timeslots', 'ip-restriction', 'user-management', 'role'];
                                                            ...
                                                            <table class="m-4 w-100">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    <tr style="border-bottom: 1px solid #eee; {{ $index % 2 == 0 ? 'background: #eee;' : '' }}">
                                                                        <td>
                                                                            <input type="checkbox" name="modules[]" value="{{ $module }}" class="module-checkbox">
                                                                        </td>
                                                                        <td class="fw-bold" style="font-weight: 700 !important">{{ ucfirst($module) }}</td>
                                                                        <td>
                                                                            <div class="row">
                                                                                @foreach($hrmpermissions as $permission)
                                                                                    @if(str_contains($permission->name, $module))
                                                                                        <div class="col-md-2">
                                                                                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" id="{{ $module.$permission->id }}" class="permission-checkbox" data-module="{{ $module }}" {{ $rolepermissions->contains('id', $permission->id) ? 'checked' : '' }}>
                                                                                            @php
                                                            ...
                                                            <script>
                                                                // Add a script to handle checkbox interactions
